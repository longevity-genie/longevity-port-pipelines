...
def test_sirt6_generic_gated_contrast_cli_is_registered_as_project_script() -> None:
    scripts = load_pyproject()["project"]["scripts"]

    assert scripts["sirt6-generic-gated-contrast"] == (
        "longevity_port_pipelines.stages.sirt6_generic_gated_contrast:app"
    )
