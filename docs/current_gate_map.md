...
- G3SX30 dry-run preflight manifest scaffold status: schema/header-only table/helper in `data/config/g3sx30_dry_run_preflight_manifest_schema.yaml`, `data/input/g3sx30_dry_run_preflight_manifest.csv`, and `src/longevity_port_pipelines/stages/g3sx30_dry_run_preflight_manifest.py`; source dry-run preflight decision row is `data/input/g3sx30_dry_run_preflight_decisions.csv#1`; no committed manifest entry, dry_run_only=`true`, max_live_batch_size=`0`, no curated_embedding_preflight run, no curated_embedding_single run, no Biohub / ESMC, no embeddings, no .npy, no ready_for_preflight runtime unlock, no Gate 8 / Gate 9, no Boltz/AF3/Chai, and no biological claim
